...
public class MySwipeRefreshLayout extends SwipeRefreshLayout {
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed) {
        ...
        if (target.getId() == R.id.tab_recycler_view && partRecyle != null) {
            RecyclerView.ViewHolder containingViewHolder = partRecyle.findContainingViewHolder(target);
            if (containingViewHolder instanceof TabSpecialViewHolder) {
                int top = containingViewHolder.itemView.getTop();
                if (top == 0 && dy > 0) {
                    return;
                }
                if (dy < 0 && target.canScrollVertically(-1) && top == 0) {//向下拉且列表没有到顶
                    return;
                }
